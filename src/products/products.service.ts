...
export class ProductsService {
  ...
  async create(createProductDto: CreateProductDto, user: User) {
    try {
      ...
      return {
        ...saveProduct,
        images: saveProduct.images.map((image) => image.url),
      };
    } catch (error) {
  ...
  async findOne(term: string): Promise<Product> {
    ...
    if (isUUID(term)) {
      ...
      product = await this.productRepository
        ...
        .select(['c', 'im.url'])
        .leftJoin('c.images', 'im')
        .where(`LOWER(c.title) LIKE :term or c.slug = :term`, {
  ...
  private handleDBExceptions(error: any) {
    if (error?.code === '23505') {
      throw new BadRequestException(error?.detail || 'Bad Request Error');
    }
    console.log(error);
